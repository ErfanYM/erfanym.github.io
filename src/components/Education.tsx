...
import type { Education as EducationEntry } from "../content/education";
      ...
      <div className="relative">
        ...
        <div className="flex flex-col gap-14">
          {education.map((entry, i) => {
            const isLeft = i % 2 === 0;
            return (
              <div key={entry.school} className="relative md:grid md:grid-cols-2 md:gap-16 items-start">
                {isLeft ? (
                  <div className="relative md:justify-self-end w-full md:max-w-md">
                    <span className="hidden md:block absolute top-9 -right-[42px] w-3 h-3 rounded-full bg-accentInk ring-4 ring-bg" />
                    <EducationCard entry={entry} />
                  </div>
                ) : (
                  <div className="relative md:col-start-2 md:justify-self-start w-full md:max-w-md">
                    <span className="hidden md:block absolute top-9 -left-[42px] w-3 h-3 rounded-full bg-accentInk ring-4 ring-bg" />
                    <EducationCard entry={entry} />
                  </div>
                )}
              </div>
